...
function preload() {
    ...
    introImage = loadImage("GamerTime.png");
}
...
function setup() {
    ...
    intro = createSprite(windowWidth / 2, windowHeight / 2);
...
function draw() {
    background("green");
    // A timer the goes through the entire game (only used for intro tho)
    introTimer += 0.1;
    ...
    if(gameState === "INTRO") {
        ...
        EXblob.x = windowWidth / 2 - 200;

        // Make the gamer time ninja grow until a certain point
        if(introTimer < 10) {
    ...
    if(introTimer >= 10 && introTimer < 25 && gameState === "INTRO") {
        fill("blue");
        textSize(60);
        text("Solo developed by Gamer Time!", windowWidth/5, windowHeight - windowHeight + 100);
        textSize(25);
        text("Subscribe to Gamer Time! on Youtube!", windowWidth/3 + 25, windowHeight - 100);
    }
}
